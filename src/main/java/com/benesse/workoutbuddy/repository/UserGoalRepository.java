...
public class UserGoalRepository {
	...
	public void changeActiveStatus(String userId) {
		String sql = "update user_goals set is_active = 'False' where user_id = ?";
		jdbcTemplate.update(sql, userId);
	}
	
	//    目標取得メソッド
	public UserGoal getLatestGoal(String userId) {
		String sql = "select * from user_goals where user_id = ? and is_active = 'True'";
		
		RowMapper<UserGoal> mapper = new DataClassRowMapper<>(UserGoal.class);
		UserGoal latestGoal = jdbcTemplate.queryForObject(sql, mapper, userId);
